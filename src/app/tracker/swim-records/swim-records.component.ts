...
@Component({
  selector: 'app-swim-records',
  standalone: true,
  imports: [CommonModule, MatExpansionModule, MatIconModule, MatTooltipModule, FireTimeRecordPipe, RouterModule],
  templateUrl: './swim-records.component.html',
  styleUrls: ['./swim-records.component.scss']
})
export class SwimRecordsComponent implements OnInit, OnDestroy {
  swimRecords: SwimRecord[] = [];
  isLoading = true;

  // Store the pre-computed records by year and month
  recordsByYearAndMonth: { [year: string]: { [month: string]: SwimRecord[] } } = {};
  ...
  ngOnInit() {
    // Load user's swim records from Firebase
    this.loadUserSwimRecords();
  }
  ...
  loadUserSwimRecords() {
    ...
    this.authService.user$.pipe(
      ...
      if (user) {
        ...
        if (userId) {
          // Subscribe to the user's swim records
          this.authService.getUserSwimRecords(userId)
            .pipe(takeUntil(this.destroy$))
            .subscribe({
              next: (records) => {
                this.swimRecords = records;
                console.log('Loaded swim records:', this.swimRecords);
                // Pre-compute records by year and month
                this.organizeRecordsByYearAndMonth();
                this.isLoading = false;
              },
              error: (error) => {
                console.error('Error loading swim records:', error);
                this.swimRecords = [];
                this.organizeRecordsByYearAndMonth();
                this.isLoading = false;
              }
        ...
        console.warn('No user logged in, cannot load swim records');
        this.swimRecords = [];
        this.organizeRecordsByYearAndMonth();
        this.isLoading = false;
      }
  ...
  organizeRecordsByYearAndMonth() {
    ...
    // Process each record
    for (const record of this.swimRecords) {
      // Parse the date
  ...
  getTotalRecordsForYear(yearData: { [month: string]: SwimRecord[] }): number {
    let total = 0;
